...
<div class="p-2rem mb-2rem border">
	{#if error}
	...
	<form on:submit|preventDefault={saveLesson} class="grid gap-4">
		...
		<div>
			...
			<input class="w-full border" bind:value={lesnummer} required />
			<small>Bijvoorbeeld: 1, 2, 3...</small>
		</div>
		<div>
			<label>Onderwerp van de les:</label>
			<input class="w-full border" bind:value={onderwerp} required />
		...
		<div>
			...
			<div class="mt-2">
				<b>Schooltype:</b>
				{getSchoolType(doelgroep_min)}
				{#if doelgroep_min !== doelgroep_max}
					&nbsp;–&nbsp;{getSchoolType(doelgroep_max)}
				{/if}
				<br />
				<b>Groep/Klas:</b>
				{getGroupLabel(doelgroep_min)}
				{#if doelgroep_min !== doelgroep_max}
					&nbsp;–&nbsp;{getGroupLabel(doelgroep_max)}
				{/if}
				<br />
				<b>Gemiddelde leeftijd:</b>
				{getAge(doelgroep_min)[0]}–{getAge(doelgroep_max)[1]} jaar
			</div>
			<small
				>Selecteer het minimum en maximum leerjaar/groep voor deze les. Groep 1-8 = basisschool,
				Brugklas/Klas 1-6 = middelbare school.</small
			>
		</div>
		<div>
			...
			<div class="flex items-center gap-4">
				<input
					...
					step="5"
					bind:value={duur}
					on:input={(e) => validateDuur(Number((e.target as HTMLInputElement)?.value))}
				/>
				<input
					type="number"
					min="15"
		...
		<div>
			...
			<textarea class="w-full border" bind:value={waarom} required></textarea>
			<small
				>Leg kort uit waarom de inhoud van deze les relevant is voor de leerlingen. Dit kan worden
				aangepast aan hun leefwereld of toekomstige doelen. Bijvoorbeeld: "In deze les leer je hoe
				je veilig online kunt communiceren, een vaardigheid die essentieel is voor sociale media en
				schoolprojecten."</small
			>
		</div>
		<div>
			<label>Doel van de les (voor leerlingen):</label>
			<textarea class="w-full border" bind:value={doel} required></textarea>
			<small
				>"Aan het einde van deze les kun je..." (bijvoorbeeld: een veilige wachtwoordstrategie
				toepassen, kritisch bronnenonderzoek doen, etc.)</small
			>
		</div>
		<div>
					...
					{#if kerndoelenSelected.length > 0}
						<ul class="ml-6 list-disc">
							{#each kerndoelenSelected as l: string}
								<li>
									{kerndoelenOpties.flatMap((g) => g.options).find((o) => o.value === l)?.label ||
										l}
								</li>
							{/each}
						</ul>
		...
		<div>
			...
			<textarea class="w-full border" bind:value={intro} required></textarea>
			<small
				>Korte uitleg van de les en de doelen. Stel een vraag of geef een voorbeeld om
				nieuwsgierigheid te wekken.</small
			>
		</div>
		<div>
			<label>Kernactiviteit (20-30 min):</label>
			<textarea class="w-full border" bind:value={kern} required></textarea>
			<small
				>Beschrijving van de hoofdactiviteit(en). Bijvoorbeeld: Zoekopdracht met een focus op
				bronbeoordeling. Hands-on oefening in het maken van een sterk wachtwoord.
				Programmeeropdracht met een eenvoudige tool zoals Scratch.</small
			>
		</div>
		<div>
			<label>Reflectie en Afronding (5-10 min):</label>
			<textarea class="w-full border" bind:value={reflectie} required></textarea>
			<small
				>Bespreek wat ze hebben geleerd en hoe dit toepasbaar is in het dagelijks leven. Stel vragen
				om te controleren of het doel is bereikt.</small
			>
		</div>
		<div>
			<label>Evaluatie van Leerlingen:</label>
			<textarea class="w-full border" bind:value={evaluatie} required></textarea>
			<small
				>Hoe toets je of de leerlingen het doel hebben bereikt? (Quiz, opdracht, groepsdiscussie,
				etc.)</small
			>
		</div>
		<div>
			<label>Feedback voor de Les:</label>
			<textarea class="w-full border" bind:value={feedback} required></textarea>
			<small>Reflectie voor de leraar: Wat ging goed? Wat kan beter?</small>
		</div>
		<div>
			<label>Aanvullende Opdrachten (optioneel):</label>
			<textarea class="w-full border" bind:value={aanvullend}></textarea>
			<small
				>Suggesties voor leerlingen die extra uitdaging nodig hebben of voor thuisgebruik.</small
			>
		</div>
		<div>
		...
		<div>
			...
			<input
				...
				placeholder="URL naar preview afbeelding (thumbnail)"
			/>
		...
		<button class="cta" type="submit" disabled={adding}
			>{adding
				? editing
					? 'Bijwerken...'
					: 'Toevoegen...'
