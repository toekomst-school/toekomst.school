<script lang="ts">
	...
	$: filteredCourses = courses.filter((course) => {
		const matchesName =
			!filterName || (course.name || '').toLowerCase().includes(filterName.toLowerCase());
		const matchesDoelgroep =
			!filterDoelgroep ||
			(course.doelgroep || '').toLowerCase().includes(filterDoelgroep.toLowerCase());
		const matchesSearch =
			!search ||
...
<div class="mb-4 flex flex-wrap gap-2">
	<input class="rounded border p-2" placeholder="Filter op cursusnaam" bind:value={filterName} />
	<input
		class="rounded border p-2"
		placeholder="Filter op doelgroep"
		bind:value={filterDoelgroep}
	/>
	<input class="rounded border p-2" placeholder="Zoeken..." bind:value={search} />
</div>
...
{:else}
	<DataTable
		...
		{columns}
		on:rowClick={(e) => goto(`/cursussen/${e.detail.$id}`)}
	/>
{/if}

<div class="p-2rem mb-2rem mt-8 border">
	...
	<form on:submit|preventDefault={addCourse} class="flex flex-col gap-2">
		...
		<textarea class="border p-2" placeholder="Beschrijving (optioneel)" bind:value={description}
		></textarea>
		...
		<input
			...
			min="0"
			step="0.01"
			placeholder="Prijs in euro (optioneel)"
			bind:value={price}
		/>
		<button class="cta w-fit" type="submit" disabled={adding}
			>{adding ? 'Toevoegen...' : 'Cursus toevoegen'}</button
		>
	</form>
</div>
